...
def top_40(): #This function heapifies the map with terms count and extracts the top 40
    file = 'sent'
    tf_map = {}
    for i in range(1,51):
        filename = file+str(i)+'.txt'
...
def create_knowledge_base(word_list): #This function is used to create the knowledge base by adding sentences which containa top 10 words
    ...
    for word in word_list:
        ...
        for i in range(1,51):
            ...
            sent_list = f.readlines()
            for s in sent_list:
                if word in s.lower() and len(knowledge_base[word])<15:
...
if __name__ == '__main__':
    ...
    print(top_40())
    my_terms = ['empire','republic','mandalorian','skywalker','kenobi','anakin','lucas','trilogy','vader', 'yoda','lightsaber','luke','solo']
    kb = create_knowledge_base(my_terms)
